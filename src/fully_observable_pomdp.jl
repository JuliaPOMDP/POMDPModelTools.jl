...
POMDPs.obs_index(pomdp::FullyObservablePOMDP{S, A}, o::S) where {S, A} = state_index(pomdp.mdp, o)
...
POMDPs.state_index(pomdp::FullyObservablePOMDP{S,A}, s::S) where {S,A} = state_index(pomdp.mdp, s)
POMDPs.action_index(pomdp::FullyObservablePOMDP{S, A}, a::A) where {S,A} = action_index(pomdp.mdp, a)
